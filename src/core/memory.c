...
uint8_t* heapEnd = (void*)0xC0000000;
...
void memoryInit(void)
{
	...
	*/
	...
	//debugPrintFormated("Heap start found at: %x\n", heapStart);
}
...
{	
	...
	while((uint8_t*)node != heapEnd && (uint8_t*)node != heapFarestAlloc)
	{
		if(node->magic == Free && node->size >= size)
		{
			if(node->size > size + sizeof(AllocNode))
			{
				...
				((AllocNode*)((uint8_t*)spliter + spliter->size))->previous = spliter;
	...
	debugPrintFormated("Found block at %x (%x)\n", node, (node + sizeof(AllocNode)));
